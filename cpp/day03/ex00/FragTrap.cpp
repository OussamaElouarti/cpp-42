...
void    FragTrap::vaulthunter_dot_exe(std::string const & target)
{
    ...
    if (this->_EnergyPoints >= 25)
    {
        ...
        if (attack[0] == 'H' || attack[0] == 'B')
            ...
            std::cout << "FR4G_TP " <<  this->_name << " attacks " << target << " with " << attack << " make " << target << " scared for 5s!" << std::endl;
        else
            std::cout << "FR4G_TP " <<  this->_name << " attacks " << target << " with " << attack << " make " << target << " stuned for 3s!" << std::endl;
    }
    else
